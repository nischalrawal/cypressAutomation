...
cy.get('#countries').select("20",{force:true})
cy.get("#default-search").click().type("Wednesday");


// cy.get('.la-trash-alt').each(($row)=>{

//   cy.wrap($row).click({force : true})

//   cy.wait(600);

//     cy.get('#delete_club_id').click({force : true});
...
function deleteDuplicateClubs() {
  cy.get('.la-trash-alt').then($trash => {
    if ($trash.length > 0) {
